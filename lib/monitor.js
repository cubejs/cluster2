...
var Monitor = module.exports = function Monitor(options) {
    ...
    app.get('/admin/v3console/ComponentStatus(/[^/]+)?', function(req, res){
        ...
        var component = res.query["component"], //should come from param
            ...
            registry = require("./componenent-status.js").componentStatusRegistry;

        if(component){
            res.writeHead(200, {
                'Content-Type': _.isEqual(view, "json") ? 'application/json' : "text/html"
            });
            registry.getStatus(component, res.query, function(result){
            ...
            res.send(JSON.stringify(registry.components()));
            res.end();
        }
    });
