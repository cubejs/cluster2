...
var debugMiddleware = exports.debugMiddleware = function(req, res, next){
	...
	if(req.url === '/status'){
		...
		//render as a view of table, each status name links to the status url of its own

		//each status url accepts 'GET' to render, and an additional view template path, to overwrite the default rendering

		//each status url also accepts 'PUT', which takes the url params to give to the update function registered of the status
...
exports.monCreateServer = function createServer(app){
	...
	var server = require('http').createServer(app),
		...
		state = null,
		statuses = {},
		sockets = [];//all of these vars are now scoped in monCreateServer to make unique
