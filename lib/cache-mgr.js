...
var common = require('./cache-common.js'),
	...
	ensureDir = require('./utils.js').ensureDir;
...
var manager = {
	...
	'all': function(conn, token, namespace){
		...
		var cache = namespaces[namespaces] || {};
...
module.exports = {
	...
	'afterServerStarted': function() { //'listening' listener
		...
		var updateTask = function updateTask(){
			...
			when.map(cachesToPersist, function(cache, namespace){

				var waitForPersist = when.defer(),
					persistence = path.join(persistPath, namespace + '.cache');
				
				fs.writeFile(persistPath, common.serialize(persists), function(err){
