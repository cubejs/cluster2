...
var Puppet = exports.Puppet = function Puppet(master, worker, options, env){
	...
	_this.emitter.once(util.format('worker-%d-warmup', _this.pid), function(){

		_this.emitter.to(['master']).emit('warmup', _this.pid);
	});
	...
	_this.forkedState = {
		...
		'whenListening': function(){
            ...
            _this.emitter.to(['self']).emit(util.format('worker-%d-listening', worker.process.pid));
		},
	...
	_this.activeState = {
		...
		'whenHeartbeat': function(heartbeat){
			...
			if(master.assertOld(heartbeat)){
				...
				deathQueue(worker.process.pid, master.emitter, function(){//success function
