		...
		'whenHeartbeat': function(heartbeat){
			...
			if(master.assertOld(heartbeat)){
				...
				var waitToDie = when.defer(),
					...
					die = function(){
						...
						master.emitter.once(util.format('worker-%d-listening', successor.process.pid), function(){
							...
							waitToDie.resolve(worker.pid);

							if(master.dyingQueue === afterDeath){//last of dyingQueue resolved, clean up the dyingQueue
				...
				if(!master.dyingQueue){//1st in the dying queue, 
					afterDeath = master.dyingQueue = timeout(waitToDie.promise, 60000);//1 min
					die();
				}
				else{
					afterDeath = master.dyingQueue = timeout(master.dyingQueue, 60000).ensure(die);
				}
			}
		},
