...
Master.prototype.initialize = function initialize(proc, options){
	...
	if(cluster.isMaster) {
			...
			'configureApp': function(monApp){
                ...
                .then(
                    function (ports) {
                    	...
                    	console.log('selected ports approved');
                        return options.monConfigureApp(monApp);
                    }, 
                    function (error) {

                        console.log('ports rejected');
		...
		var CacheMgrWorker = require('./cache-mgr-worker').CacheMgrWorker,
		...
		return new CacheMgrWorker(proc, options);
	}
	else{
...
Master.prototype.fork = function fork(options, env){
    ...
    if (env.CACHE_MANAGER) {
        utils.pickAvailablePorts(_this.port + 10, _this.port + 30, 2).then(function (ports) {
            _this.logger.info('[master] pick up ports %j for cache manager', ports);
            env.CACHE_PORTS = JSON.stringify(ports);
            tillFork.resolve(cluster.fork(env));
        }).otherwise(function (error) {
            tillFork.reject(error);
        });
    }else {
        tillFork.resolve(cluster.fork(env));
    }

    return tillFork.promise.then(function (forked) {
