	...
	if(cluster.isMaster) {
		...
		_.extend(_this, {
			...
			'markOld': options.markOld || function(heartbeat){
				...
				var thisTPS = heartbeat.durations.dividedBy(heartbeat.transactions),
					historicTPS = heartbeat.totalDurations.dividedBy(heartbeat.totalTransactions);
			...
			'status': require('./cluster-status.js').componentStatus
		});

		cluster.on('fork', function(worker){
			_this.slaves[worker.process.pid] = new Slave(_this, worker, options, worker.process.env);
		});
...
Master.prototype.status = function status(){
...
Master.prototype.whenStop = function whenStop(){
	...
	var _this = this,
		...
		gracefully = function(){
			...
			if(_.keys(_this.slaves).length === 0){
				process.exit(0);
			}
 ...
 */
var Slave = exports.Slave = function(master, worker, options, env){
