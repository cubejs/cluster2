...
Master.prototype.initialize = function(proc, options){
	...
	if(cluster.isMaster) {
		...
		when.all([
			...
			exitIfBusyPort('localhost', _this['debug.debugPort']),
			exitIfBusyPort('localhost', _this['debug.webPort'])
		])
		.otherwise(function(){

			_this.logger.error('[master] one of the ports:%j we need has been occupied, please shutdown your program on that port',
				[_this.port, _this.monPort, _this['debug.debugPort'], _this['debug.webPort']]);
...
Master.prototype.debug = function debug(pid){
	...
	ready.then(function(){//must wait for the signal to be sent, and child process ready to accept connection on 5858
		...
		var inspector = execFile(inspectorPath, inspectorArgs),
			buff = '';
