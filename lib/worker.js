...
var Worker = exports.Worker = _.once(function(proc, options){
	...
	if(_this.gc.monitor){
		...
		gcstats.on('stats', function(stats) {
			...
			_this.whenGC(stats.after.usedHeapSize, stats.pauseMS < 500 ? 'incremental' : 'full');
		});
	}
	...
	process.once('SIGUSR1', function(){
		process.nextTick(function(){
			emitter.emit(util.format('debug-%d-listening', _this.pid), ['master']);
		});
	});
...
Worker.prototype.listen = function(){
	...
	var deferred = when.defer(),
		run = function(){
			...
			configureApp(app); //configure app before listening
			...
			var server = createServer(app).listen(port, function(){
				...
				when(warmUp()).ensure(function(){ //warm up app after listening

					_this.emitter.emit(util.format('worker-%d-warmup', _this.pid), ['master', 'self']); //tell master i'm ready
...
Worker.prototype.whenGC = function(usage, type){
	...
	_this.logger.debug('[worker] gc usage:%s, type:%s', usage, type);
