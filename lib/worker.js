...
Worker.prototype.listen = function listen(){
		...
		run = function(){
			...
			when(configureApp(app)).ensure(function(configured){ //configure app before warming up
                ...
                var warmUpServer = createServer(app).listen(warmUpPort, function(){ //warming up by starting listening on `warmUpPort`
                    ...
                    when(warmUp(app, warmUpServer.address())).ensure(function(){ //notify the `warmUp` callback, server is listeing at `address`
						...
						warmUpServer.close(function(){ //switching listening port from `warmUpPort` to the actual `port`
                            ...
                            var server = _this.server = createServer(app).listen(port, function(){
					            ...
					            _this.emitter.emit(util.format('worker-%d-listening', _this.pid), server.address()); //tell master, worker ready
    ...
    if (process.env.CACHE_MANAGER) {
        run = function () {
            var server = createServer(app).listen(port, null, function() {
                _this.logger.info('[cache] manager %d started listening on %j', _this.pid, port);
                when(warmUp()).ensure(function () {
                    _this.logger.info('[cache] manager %d warmed up', _this.pid);
                    _this.emitter.emit(util.format('worker-%d-warmup', _this.pid)); //tell everyone warmup is done
                    _this.emitter.emit(util.format('worker-%d-listening', _this.pid), port); //tell master, worker ready
...
Worker.prototype.whenStop = function whenStop(){
...
Worker.prototype.whenExit = function whenExit(){
