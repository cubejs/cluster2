...
var Worker = exports.Worker = _.once(function(proc, options){
	...
	memwatch.on('stats', function(stats){
		...
		if(stats['num_full_gc'] > _this.historicFull || stats['num_inc_gc'] > _this.historicIncremental){
			_this.whenGC(stats);
		}
	});
...
Worker.prototype.whenGC = function(stats){

	var _this = this,
		full = stats['num_full_gc'] - _this.historicFull,
		incr = stats['num_inc_gc'] - _this.historicIncremental,
		type = full ? 'full' : 'incremental';

	_this.logger.info('[worker] gc usage:%j, type:%s', usage, type);

	_this.gc[type] += 1;
	_this.historicFull = full;
	_this.historicIncremental = incr;
