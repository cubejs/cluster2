...
var Worker = exports.Worker = function(proc, options){
	...
	_.extend(_this, {
		...
		'app': options.app,
		'port': options.port,
        'warmUpPort': process.env.warmUpPort,
	...
	var tillListen = when.defer(),
		run = function(){
			...
			when(configureApp(app)).ensure(function(configured){ //configure app before listening
                ...
                var warmUpServer = createServer(app).listen(warmUpPort, function(){
                    
                    _this.logger.info('[worker] %d started warming on:%d', _this.pid, warmUpPort);
                    //_this.logger.info('[worker] %d %j', _this.pid, warmUpServer.address());
                    _this.emitter.to(['master', 'self']).emit(util.format('worker-%d-warming', _this.pid));
                    
                    when(warmUp(app, warmUpServer.address())).ensure(function(){ //warm up app after listening
						...
						warmUpServer.close(function(){
                            ...
                            var server = _this.server = createServer(app).listen(port, function(){
					            ...
					            _this.emitter.to(['master', 'self']).emit(util.format('worker-%d-listening', _this.pid), server.address());
