            ...
            if(message.type === "heartbeat"){
                ...
                if(expect){//there're jobs which expects immediate responses, in case of those, onExpect handler is created, timeout might be applied
                    ...
                    timeOut = setTimeout(function(){
                        console.log('[cluster] reject timeout:' + JSON.stringify(message));
                    ...
                    deferred.promise
                        ...
                        .fail(function(error){
                            console.log('[cluster] fail error:' + error);
                            message.error = error;
                            send(message);
                        })
                        .fin(function(){
    ...
    this.notifyWorkers = function(message) {
        _.each(self.workers, function(worker, pid) {
            try{
                ...
                console.log('[cluster2] cannot send message to worker:' + pid);
            } 
        });
    }
}
...
Process.prototype.listen = function() {
    ...
    process.exit = function(){
        console.log('[cluster2] exit unexpectedly' + new Error().stack);
    ...
    if(cluster.isMaster) {
        ...
        monitor.once('listening', function() {
            ...
            var deathWatcher = function (worker, code, signal) {
                ...
                console.log('[cluster2] death watch activated, worker:' + worker.pid + '\tcode:' + code + '\tsignal:' + signal + '\texit:' + worker.exitCode);
                if(code === 0) {
                ...
                console.log('[cluster2] updated worker list:' + _.keys(self.workers));
            };
        ...
        var recycle = setInterval(function() {
            if(totalConns > threshold) {
                console.log('[cluster2] exit because of connection threshold:' + threshold + ':' + totalConns);
