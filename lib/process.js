    ...
    this.createWorker = function () {
        ...
        worker.on('message', function (message) {
            ...
            if(message.type === 'heartbeat'){
                ...
                var prepare = self.createWorker();
                self.workers[prepare.pid + ''] = prepare;

                var tillPrepared = when.defer();
                self.emitter.on('listening', function onListen(pid){

                    if(prepare.pid === pid){
                        self.emitter.removeListener('listening', onListen);
                        tillPrepared.resolve(pid);
                    }
                });

                timeout(60000, tillPrepared.promise).ensure(function(){//after one minute or the prepared replacement is listening, send 'SIGINT' to the suicidal worker

                    //let the worker suicide now
                    worker.kill('SIGINT');
                });
            }
