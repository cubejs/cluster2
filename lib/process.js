...
Process.prototype.listen = function() {
    ...
    if(cluster.isMaster) {
        ...
        function getWarmupPort() {
            var basePort = self.options.warmup.basePort || 9000;
            return basePort + process.pid % 100;
        }

        function warmup(server, host) {
            if (self.options.warmup && self.options.warmup.urls && self.options.warmup.urls.length > 0) {
                // workers need warmup
                // listen to a random port
                server.listen(getWarmupPort(), host);
                server.on('error', function (err) {
                    process.exit(-1);
                });
                server.once('listening', function () {
                    var warmupPort = server.address().port;
                    var warmupUrls = self.options.warmup.urls;
                    console.log(util.format('Worker:%s listens to %s', process.pid, warmupPort));
                    // warming up
                    var promises = _.map(warmupUrls, function(path) {
                        var deferred = when.defer();
                        var promise = deferred.promise;
                        console.log('http://' + host + ':' + warmupPort + path);
                        request.get('http://' + host + ':' + warmupPort + path, function (err, res, body) {
                            if (err) {
                                deferred.reject('[warmup ' + path + ']'+err);
                            }else if (res.statusCode !== 200) {
                                deferred.reject(util.format('[Warmup Error]worker:%s got %s for %s', process.pid, res.statusCode, path));
                            }else {
                    ...
                    when.all(promises)
                        .then(function (values) {
                            // start the listening to the correct port
                            console.log(util.format('Woker:%s finished warmup', process.pid))
                            //console.log(values);
                            server.close();
                            self.emitter.emit('warmup-finished-' + process.pid);
                        })
        ...
        _.each(apps, function(app){
            ...
            self.emitter.on('warmup-finished-' + process.pid, function () {
