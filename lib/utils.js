...
exports.getLogger = (function(dir){
    ...
    //to avoid the annoying error, winston registers too many 'error' handler on the same transport instance.
    fileLoggerTransport.setMaxListeners(1024);
...
exports.deathQueueGenerator = function(options){
	...
	return function deathQueue(queue, pid, emitter, success){
			...
			var tillDeath = when.defer(),
				...
				die = function die(retry){
					...
					var successor = success(),
                        ...
                        successorEvent = util.format('worker-%d-listening', successorPid),
                        successorGuard = setTimeout(function onSuccessorTimeout(){
                            ...
                            emitter.removeListener(successorEvent, onSuccessorListening);
					...
					emitter.once(successorEvent, function onSuccessorListening(){
						...
						emitter.to(['master', pid]).emit('dismiss', pid);
...
exports.markUpAfterAllListening = function markUpAfterAllListening(emitter, expects){
	...
	return when.map(expects, function(pid){
		...
		var tillListening = when.defer();
        
        emitter.once(util.format('worker-%d-warmup-failure', pid), function(error){
        
            tillListening.reject(error);
        });
		
		emitter.once(util.format('worker-%d-listening', pid), function(address){
			
			tillListening.resolve(address);
		});

		return timeout(60000, tillListening.promise);
	});
};
