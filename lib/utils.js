	...
	return function deathQueue(pid, emitter, success){
		...
		if(!_.contains(queued, pid)){
			...
			if(!tillPrevDeath){//1st in the dying queue,
				afterDeath = tillPrevDeath = timeout(60000, tillDeath.promise).ensure(function(){
				...
				afterDeath = tillPrevDeath = tillPrevDeath.ensure(function(){

					setTimeout(die, waitExtra);
				});
			}
		}
	};
