...
exports.warmUpThenMark = function warmUpThenMark(emitter, expects){
	...
	var tillWorkersWarmUp = when.defer(),
		warmUpCountdown = expects;

	emitter.on('warmup', function(pid){
		
		warmUpCountdown -= 1;
		
		if(!warmUpCountdown){
			tillWorkersWarmUp.resolve(true);
		}
	});

	return timeout(60000, tillWorkersWarmUp)
};
...
exports.npmls = (function npmls(){
	...
	console.log('execPath:%s\nexecArgv:%j', execPath, execArgv);
