...
exports.rejectIfPortBusy = function rejectIfPortBusy(host, port){
    ...
    return timeout(2000, deferred.promise);
};
...
exports.startInspector = function(webPort, debugPort, saveLiveEdit, hidden, logger){
	...
	var inspectorPath = exports.getNodeInspectorPath(),
		inspectorArgs = [
			...
			'--hidden=' + hidden//files excluded from adding breakpoints
		];

	assert.ok(inspectorPath);
	
	if(logger){
		logger.info('[debug] starting node-inspector at:%s with args:%j', inspectorPath, inspectorArgs);		
	}
