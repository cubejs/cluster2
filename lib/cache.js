...
module.exports = {
	...
	'enable': _.once(function(options, master){
		...
		if(!options.enable || !require('cluster').isMaster || (process.cluster2 && process.cluster2.caching)){
			//only master is allowed to enable cache, caching could be enabled only once
			return;
		}
		...
		if(options.mode === 'standalone' && master){
			...
			return master.fork(master.options, {
				'CACHE_MANAGER': true,
            ...
            var basePort = master ? master.port + 10 : 9190;
            
            return utils.pickAvailablePorts(basePort, basePort + 20, 2).then(function (ports) {
                process.env.CACHE_PORTS = JSON.stringify(ports);
